...
This module converts markdown files to an HTML string for use by the Dancer2 web
app framework. Using the Dancer2 config file, multiple routes can be established
in the web app, with each route converting a single markdown document or all the
markdown documents in a directory to HTML. Optionally, it can return a second
HTML string containing a hierarchical table of contents based on the contents of
the markdown documents. The HTML will be displayed with the templates and routes
specified in Dancer2's config file. See the C<CONFIGURATION> section for more
details. To generate HTML from within your app's code, refer to the C<KEYWORDS>
section.
...
Modifying the Dancer2 C<config.yml> file to associate routes with the
...
generated by this module in some way, using the configuration file to generate
the HTML is preferred over using keywords to.
...
maintained. Suggestion for improvement are welcome.
